...
tf.app.flags.DEFINE_float('ALPHA', 0.5,
                          'learning rate for embedding centers')
tf.app.flags.DEFINE_float('LAMBDA', 0.,
                          'scalar to multiply with the center loss')
...
def train(hps):
  ...
  logging_hook = tf.train.LoggingTensorHook(
      tensors=OrderedDict(zip(['iterations', 'total loss', 'cross entropy', 'weight decay', 
                               'center loss', 'accuracy'],
                      [model.global_step, model.cost, model.xent_cost, 
                       model.decay_cost, model.center_loss, precision])),
      every_n_iter=100)
  ...
  class _LearningRateSetterHook(tf.train.SessionRunHook):
    ...
    def before_run(self, run_context):
      return tf.train.SessionRunArgs(
          ...
          model.decay_cost, model.center_loss],
    ...
    def after_run(self, run_context, run_values):
      ...
      center_loss = run_values.results[5]
      ...
      train_metrics_dict['center_loss'].append(center_loss)
