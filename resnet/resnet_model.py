...
HParams = namedtuple('HParams',
                     ...
                     'num_residual_units, use_bottleneck, weight_decay_rate, ALPHA, '
                     'LAMBDA, relu_leakiness, optimizer')
...
class ResNet(object):
  ...
  def _build_model(self):
    ...
    with tf.variable_scope('costs'):
      ...
      self.center_loss = self._center_loss_fn(embeddings, self.labels)
      self.cost = self.xent_cost + tf.multiply(self.hps.weight_decay_rate, self.decay_cost) + tf.multiply(self.hps.LAMBDA, self.center_loss)
